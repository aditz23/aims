...
<h1 style="text-color:white;font-size:200%;text-align:center;"><strong>INSERT INTO OTHERS TABLE</strong></h1>
    <div class="row">
...
$val=mysqli_query($con,"select * from others where plot='$plot' ");
$count=mysqli_num_rows($val);
if($_REQUEST['button']=='Insert')
{
	mysqli_query($con,"insert into others values('$country','$language','$plot','$imdb_score')");
}
$query=mysqli_query($con,"select * from  others where plot='$plot'");
echo "<table class='styled' border=\"1\">";
echo "<tr><td class='head'>"."country"."</td><td class='head'>"."language"."</td><td class='head'>"."plot"."</td><td class='head'>"."imdb_score"."</td><td>"."</tr></td>";
while($row=mysqli_fetch_array($query))
{
	echo "<tr><td class='ele'>".$row['country']."</td><td class='ele'>".$row['language']."</td><td class='ele'>".$row['plot']."</td><td class='ele'>".$row['imdb_score']."</td><td>"."</tr></td>";
}
echo "</table>";
mysqli_close($con);
?>
