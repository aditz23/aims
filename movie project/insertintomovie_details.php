...
<h1 style="text-color:white;font-size:200%;text-align:center;"><strong>INSERT INTO MOVIE DETAILS TABLE</strong></h1>
    <div class="row">
...
$genre=$_GET["genre"];
$val=mysqli_query($con,"select * from movie_details where movie_title='$movie_title' ");
$count=mysqli_num_rows($val);
if($_REQUEST['button']=='Insert')
{
	mysqli_query($con,"insert into movie_details values('$movie_title','$director_name','$actor_name1','$genre')");
}
$query=mysqli_query($con,"select * from  movie_details where movie_title='$movie_title'");
echo "<table class='styled' border=\"1\">";
echo "<tr><td class='head'>"."movie_title"."</td><td class='head'>"."director_name"."</td><td class='head'>"."actor_name1"."</td><td class='head'>"."genre"."</td><td>"."</tr></td>";
while($row=mysqli_fetch_array($query))
{
	echo "<tr><td class='ele'>".$row['movie_title']."</td><td class='ele'>".$row['director_name']."</td><td class='ele'>".$row['actor_name1']."</td><td class='ele'>".$row['genre']."</td><td>"."</tr></td>";
}
echo "</table>";
mysqli_close($con);
?>
