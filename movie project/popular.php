...
<h1 style="text-color:white;font-size:200%;text-align:center;"><strong>VIEW OF POPULAR TABLE</strong></h1>
    <div class="row">
...
if($_REQUEST['button']=='Viewpopular')
{
$query=mysqli_query($con,"SELECT review.director_name, review.num_user, review.num_critic,  popular.tot_usr
FROM review
INNER JOIN popular ON review.director_name = popular.director_name;");
echo "<table class='styled' border=\"1\">";
echo "<tr><td class='head'>"."director_name"."</td><td class='head'>"."num_user"."</td><td class='head'>"."num_critic"."</td><td class='head'>"."tot_usr"."</td><td>"."</tr></td>";
while($row=mysqli_fetch_array($query))
{
	echo "<tr><td class='ele'>".$row['director_name']."</td><td class='ele'>".$row['num_user']."</td><td class='ele'>".$row['num_critic']."</td><td class='ele'>".$row['tot_usr']."</td><td>"."</tr></td>";
}
echo "</table>";
echo "</form>";
}
